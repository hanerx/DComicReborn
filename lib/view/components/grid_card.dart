...
class GridCard extends StatelessWidget {
  ...
  /// Column count used on compact panes; wider panes derive more columns
  /// from the available width (see [AppLayout.gridColumns]) so covers stay
  /// cover-sized instead of stretching.
  final int crossAxisCount;

  /// Cell aspect ratio for grids that lay out plain cells instead of covers.
  /// When set, the adaptive column count applies to this path too.
            ...
            GridView(
              ...
              gridDelegate: childAspectRatio != null
                  ? SliverGridDelegateWithFixedCrossAxisCount(
                      // Plain cells adapt here; cover grids adapt inside
                      // GridCardItem.coverGridDelegate, so each path adapts
                      // exactly once.
                      crossAxisCount: AppLayout.gridColumns(
                        constraints.maxWidth - 32,
                        targetWidth: coverAspectRatio > 1 ? 260 : 160,
                        compactColumns: crossAxisCount,
                      ),
                      childAspectRatio: childAspectRatio!,
                      crossAxisSpacing: 12,
                      mainAxisSpacing: GridCardItem.gridSpacing,
                    )
                  : GridCardItem.coverGridDelegate(
                      context,
...
class GridCardItem extends StatelessWidget {
  ...
  static double reservedTextExtent(
  ...
  ///
  /// [crossAxisCount] is the compact-pane column preference: panes narrower
  /// than [AppLayout.navigationBreakpoint] keep it (clamped for tiny widths),
  /// while wider panes derive more columns from [gridWidth] so tablet covers
  /// stay cover-sized instead of stretching.
  static SliverGridDelegateWithFixedCrossAxisCount coverGridDelegate(
    ...
    var columns = AppLayout.gridColumns(
      gridWidth,
      targetWidth: coverAspectRatio > 1 ? 260 : 160,
      compactColumns: crossAxisCount,
      spacing: crossAxisSpacing,
    );
    var columnWidth = (gridWidth - crossAxisSpacing * (columns - 1)) / columns;
    return SliverGridDelegateWithFixedCrossAxisCount(
      crossAxisCount: columns,
      mainAxisSpacing: mainAxisSpacing,
      crossAxisSpacing: crossAxisSpacing,
      mainAxisExtent:
          columnWidth / coverAspectRatio +
          reservedTextExtent(
            context,
            hasTitle: hasTitle,
